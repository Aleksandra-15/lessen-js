...
const renderGallery = (items) => {
  ...
  gallery.innerHTML = items.map((item) => createGalleryItem(item)).join("");
};
...
const captionText = document.querySelector("captio");
const span = document.getElementsByClassName("close")[0];

document.querySelectorAll("preview original description").forEach((preview) => {
  preview.addEventListener("click", function () {
    modal.style.display = "block";
    modalImg.src = this.original;
    captionText.innerHTML = this.getAttribute(description);
  });
});
